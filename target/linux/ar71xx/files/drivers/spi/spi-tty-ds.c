...
static DEVICE_ATTR(transfer_delay, 0644, spi_tty_delay_show, spi_tty_delay_store);
...
static int __ath79_gpio_get_int_pending(void)
{
				   ...
				   struct spi_tty_message *avr_msg)
{
	 ...
	 */
	tty_insert_flip_string(tty->port, avr_msg->rx_buf + 1, valid_len);
...
/*	wake_up_interruptible(&tty->port->close_delay);*/
}
				      ...
				      unsigned int gpio_irq)
{
	...
	dev_err(&avr->spi->dev, "setting gpio %u to interrupt on rising edge\n", avr->gpio_irq);
	__ath79_gpio_set_int_polarity(avr->gpio_irq, 1);

/*	err = request_irq(avr->spi->irq, spi_tty_data_ready_int, SPI_TTY_IRQ, */
	err = request_irq(avr->gpio_irq, spi_tty_data_ready_int, SPI_TTY_IRQ,
			  KBUILD_MODNAME, avr);
...
static int spi_tty_probe(struct spi_device *spi)
{
	...
	avr->tty_dev = tty_port_register_device_attr(&avr->port,
			spi_serial_tty_driver, avr->tty_minor, &avr->spi->dev,
			NULL, spi_tty_attr_groups);
